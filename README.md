...
| 任务目录迁移 | 设置里可修改任务目录（v3.4.0）：原生目录选择器 + 可选迁移（机内任务移动、外部任务重注册），配置持久化到 userData/settings.json，支持一键重启 |
...
- 任务目录可在 **设置 → 数据** 里修改（v3.4.0）：可选迁移现有机内任务到新目录，选择保存在 `userData/settings.json`，重启后完全生效；流程目录不受影响，仍在程序数据目录下
